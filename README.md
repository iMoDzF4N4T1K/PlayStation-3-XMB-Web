...
### [View Live Demo!](https://imodzf4n4t1k.github.io/PlayStation-3-XMB-Web/)

---

## HybrideLabs Web Edition (My Web Version)

This repository contains my **HybrideLabs Web Edition** layer on top of the waves renderer:
- **Settings menu with tabs**: **General** + **Waves**
- Designed for **GitHub Pages (project page)**: paths are handled to work under `/PlayStation-3-XMB-Web/`
- **Website overlay ready**: waves can run fullscreen in the background while your website UI sits above it
- **Assets support** via `/img/` folders:
  - `img/graphics/`
  - `img/icons/`
  - `img/users/`
  - `img/videos/`
- **Important icon rule**:
  - **Dark mode** → icons should appear **white**
  - **Light mode** → icons should appear **black**
  - Recommended: keep UI icons as white PNGs and invert in light mode with CSS (brand/social icons can stay unfiltered)

> The Web Edition packaging, site structure, Settings UI and asset integration are maintained by **iMoDzF4N4T1K (HybrideLabs)**.
...
While this is inspired by the official PlayStation 3 XMB background wave design, the major starting point was from this [CodePen by Alphardex](https://codepen.io/alphardex/pen/poPZNwE). All of the modern optimizations and cleanups have been made from this code !

### Base waves engine + Wave Configuration UI (upstream)
This web edition uses/derives from **PlayStation-3-XMB** by **linkev** (MIT), including the **wave renderer** and the original **Wave Configuration interface**:
- https://github.com/linkev/PlayStation-3-XMB

> Web Edition note: this repo is a HybrideLabs web packaging/integration of the project for static hosting + Settings UI.
...
- **HybrideLabs Web Edition**:
  - **Settings tabs** (General + Waves)
  - **Static hosting friendly** (GitHub Pages)
  - **Asset folders** (`img/graphics`, `img/icons`, `img/users`, `img/videos`)
  - **Overlay usage** (waves as background behind a site)

## Local Development

1. Clone the repository:
```bash
git clone https://github.com/iMoDzF4N4T1K/PlayStation-3-XMB-Web.git
cd PlayStation-3-XMB-Web
```
...
> Note: ES modules require HTTP — do not run via `file://`.
...
- "img" (HybrideLabs Web Edition assets):
  - `img/graphics/` (visuals / UI graphics)
  - `img/icons/` (menu + social + UI icons)
  - `img/users/` (avatars / placeholders)
  - `img/videos/` (media assets)
...
### Using as a website background (HybrideLabs Web Edition)
- Run the waves canvas fullscreen behind everything
- Put your site UI above it (higher z-index)
- Use the **Settings → General** tab for site overlay options
- Use the **Settings → Waves** tab for wave parameters
...
### [Open an issue on GitHub!](https://github.com/iMoDzF4N4T1K/PlayStation-3-XMB-Web/issues)
